...
class mod_login
{
    ...
    public static function verify_login_in($username, $password)
    {
        $password_md5 = md5($password);
         //$select = "select * from ylmf_admin_user where `name`='$username' and `password`='$password_md5'";
         $select = "select * from ylmf_admin_user where `name`='$username'";
         app_db::query($select);
         $row_info = app_db::fetch_one();

         if ($row_info['password'] == $password_md5)
         {
             $auth_key = self::get_user_agent();
             $auth_password = $password;
             $auth_username = $username;
    ...
    public static function menu($id)
    {
        ...
        $menu = array
            ...
            (
                ...
                '模板编辑' => array
                (
                    '首页普通模板' => '?c=template_manage&action=modify&filename=index.tpl',
                    '首页宽屏模板' => '?c=template_manage&action=modify&filename=kp.tpl',
                    '分类模板' => '?c=template_manage&a=template_list&folder=class',
                    '专题模板' => '?c=template_manage&a=template_list&folder=topic',
                    '城市模板' => '?c=template_manage&a=template_list&folder=city',
            ...
            (
                '城市导航' => array
                (
                    '城市管理' => '?c=city_cityclass',
                    '名站管理' => '?c=city_mingzhan',
                    '酷站分类' => '?c=city_coolclass',
                    '酷站网址' => '?c=city_coolsite',
                ),
            ),
            '114啦联盟' => array
            (
