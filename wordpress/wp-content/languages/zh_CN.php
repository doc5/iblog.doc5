...
function zh_cn_l10n_load_patches_states() {
	...
	if ( false == $_zh_cn_l10n_preference_patches ) {
		...
		$_zh_cn_l10n_preference_patches = maybe_unserialize( $_zh_cn_l10n_preference_patches ); // XXX figure out why $_zh_cn_l10n_preference_patches sometimes is an array
	}
...
function zh_cn_l10n_add_options_page() {
	add_options_page( '中文本地化功能选项', '中文本地化', 'administrator',
...
function _zh_cn_l10n_info( $category, $type, $additional_vars = null ) {
	$patches_info = array(
		...
		array(
			'id'						=> 'chinese-dashboard-style',
			'name'					=> '控制板样式优化',
			'description' 	=> '微调控制板（后台）英文默认 CSS 样式表，应用针对中文的样式优化。',
			'default_state'	=> true,
			'post_enable'		=> '在您下次访问其它控制板页面时将自动应用中文样式，站点前台样式不会变化。',
 ...
 *
 * Detect if the plugins we shipped are activated, and display corresponding
 * options, e.g. activate/deactivate them.
 *
 * @since 3.3.0
 */
...
/**
 * Sets contextual help
 * 
 * Modify contextual help string.
 *
 * @since 3.3.0
 */
function zh_cn_l10n_contextual_help() {
	add_contextual_help( 'settings_page_' . ZH_CN_L10N_OPTIONS_PAGE_MENU_SLUG, 
...
function zh_cn_l10n_patch_asian_character_count_register_script() {
	...
	wp_register_script( 'word-count', $path_to_word_count_js, array( 'jquery' ), '1.0' );
}
...
/**
 * PATCH: Chinese Dashboard Style - enqueue stylesheet
 *
 ...
 * @since 3.3.0
 */
function zh_cn_l10n_patch_chinese_dashboard_style_enqueue_stylesheet() {
	$path_to_dashboard_css = WP_CONTENT_URL . '/languages/zh_CN-dashboard.css';
	wp_register_style( 'zh-cn-l10n-dashboard', $path_to_dashboard_css, array( 'wp-admin' ), '1.0');
	wp_enqueue_style( 'zh-cn-l10n-dashboard' );
}
...
function zh_cn_l10n_contextual_help_tab( $current_screen ) {
	if ( $current_screen->id == 'settings_page_' . ZH_CN_L10N_OPTIONS_PAGE_MENU_SLUG ) {
		...
		$general  = '<p>感谢选择 WordPress China 提供的官方 WordPress 中文版本。本版本内置对 WordPress 的<!-- XXX - KEEP THIS UPDATED -->三个<!-- XXX - KEEP THIS UPDATED -->补丁，您可以在本页面决定是否使用它们。</p>';
		...
		$general .= '<p><strong>控制板样式优化</strong> - 针对中文进行的控制板（后台）字体、字号优化。</p>';
		...
		// Chinese dashboard style
		$chinese_dashboard_style  = '<p>针对中文进行的控制板（后台）字体、字号优化。</p>';
		$chinese_dashboard_style .= '<p>启用后不需您的干预，自动修改样式。这个补丁不会修改您的站点前台样式。</p>';
		
		$current_screen->add_help_tab( array(
			'id'      => 'zh-cn-l10n-chinese-dashboard-style',
			'title'   => '控制板样式优化',
			'content' => $chinese_dashboard_style
		) );
...
function _zh_cn_l10n_options_page_pointer_print_admin_bar() {
	...
	$pointer_content .= '<p>点击左上方的“帮助”来查看详情以及功能用法。</p>';
...
// Dashboard style
if ( in_array( 'chinese-dashboard-style', $zh_cn_l10n_patches_enabled ) )
	add_action( 'admin_init', 'zh_cn_l10n_patch_chinese_dashboard_style_enqueue_stylesheet' );
